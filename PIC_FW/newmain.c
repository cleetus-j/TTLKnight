...
void uart_init(void)
{
    ...
    BRGH = 1;
    SYNC = 0;
    SPEN = 1;    // enable serial port (configures RC6/RC7)
...
void do_help(void)
{
    ...
    uart_puts(" set <port> <value> - set port (B,C,D) to value\r\n");
    uart_puts(" help - show this help\r\n");
    uart_puts(" set <port> <value> - set port (A,B,C,D) to value\r\n");
    uart_puts(" upp. port letters work");
    uart_puts("RA>3F");
}
...
void process_message(void)
{
    ...
    // SET command: set <port> <value> (binary, hex, or decimal)
    else if (strncmp(rx_buf, "set ", 4) == 0)
    {
        const char *arg = rx_buf + 4;
        char port = *arg++;              // first char is port letter
        if (*arg == ' ') arg++;          // skip single space after port
        unsigned char value = 0;

        // Parse value (binary, hex, decimal)
        if (arg[0] == 'b' || arg[0] == 'B')
        {
            value = (unsigned char)parse_binary(arg + 1);
        }
        else if ((arg[0] == '0' && (arg[1] == 'x' || arg[1] == 'X')) || hex_digit(arg[0]) >= 0)
        {
            value = (unsigned char)parse_hex(arg);
        }
        ...
        // Apply value to selected port
        switch (port)
        {
            
            case 'A':
                PORTA = value;  // Apply value to PORTA (RA0?RA5)
                break;
            ...
            case 'C':
                // SAFETY: Preserve UART pins RC6(TX) and RC7(RX) (bits 6,7)
                // We only modify bits 0..5; leave 6..7 untouched so UART keeps working.
                PORTC = (PORTC & 0xF8) | (value & 0x07);
                break;
...
ADCON1 = 0x06;    // Disable analog inputs, make PORTA digital
TRISA = 0x00;     // Set PORTA as output
PORTA = 0x00;     // Clear PORTA

    // Minimal change: add PORTB only. Do NOT touch TRISC to avoid breaking UART on RC6/RC7.
